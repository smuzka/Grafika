...
{
	Img_Cpy = Img_Org.Copy();
	unsigned int size = 3 * Img_Cpy.GetSize().GetWidth() * Img_Cpy.GetSize().GetHeight();
	unsigned char* orgData = Img_Cpy.GetData();
	...
	Img_Cpy.SetData(newData, Img_Org.GetWidth(), Img_Org.GetHeight());
	m_scrolledWindow->ClearBackground();
